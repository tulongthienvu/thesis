...
%Trong những năm qua, các mô hình mạng nơ-ron nhân tạo trong đã trở nên mạnh mẽ và đạt được những kết quả "state-of-the-art" trong nhiều bài toán học máy. Thành công này đến từ khả năng tự học các đặc trưng ẩn bên dưới của dữ liệu của mạng nơ-ron, không như những phương pháp truyền thống dựa trên các đặc trưng được tạo thủ công (handcrafted features). Ngày nay, giá cả của các thiết bị vi xử lý đã trở rẻ hơn, các bộ dữ liệu trở nên lớn hơn và lĩnh vực tính toán song song đã phát triển đáng kể. Điều đó khiến cho việc huấn luyện và áp dụng các mạng nơ-ron trong thực tế trở nên dễ dàng hơn rất nhiều so với trước đây. Mạng nơ-ron ngày nay đã trở thành mô hình tiêu chuẩn và được áp dụng trong nhiều ứng dụng.
 
% Mặc dù mạng nơ-ron truyền thống đã thể hiện sức mạnh trong nhiều bài toán, chúng cũng có những hạn chế. Đáng chú ý nhất, mạng nơ-ron truyền thống hoạt động dựa trên giả định rằng các mẫu dữ liệu là độc lập với nhau. Sau mỗi thời điểm một mẫu dữ liệu được xử lý, trạng thái hiện tại của mạng biến mất, các thiết lập của mạng lại trở về như ban đầu. Nếu các mẫu dữ liệu được tạo ra một cách độc lập, việc này là bình thường. Nhưng nếu các mẫu dữ liệu có liên hệ với nhau theo một thứ tự, điều này là một vấn đề. Ví dụ như các khung hình từ video, đoạn âm thanh hay các từ trong một câu là những loại dữ liệu mà ta không thể sử dụng giả định về độc lập. Thêm vào đó, những mạng nơ-ron truyền thống thường được thiết kế với các đầu vào là những vec-tơ có độ dài cố định. Điều này hạn là một hạn chế lớn với những bài toán dữ liệu đầu vào có độ dài chưa biết trước.
...
%Hình \ref{fig_rnn_loop} thể hiện kết nối vòng của RNN.
